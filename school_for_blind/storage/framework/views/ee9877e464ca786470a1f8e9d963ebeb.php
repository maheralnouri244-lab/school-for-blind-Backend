<?php $__env->startSection('content'); ?>
    ...
    <div class="row g-4">
      ...
      <div class="col-xl-4">
        ...
        <div class="custom-card mb-4 shadow-sm-hover border" style="border-color: var(--border-color) !important;">
          ...
          <div class="mb-3">
            <span class="text-muted d-block small mb-1">عنوان الامتحان</span>
            <strong class="fs-5" style="color: var(--text-main);"><?php echo e($exam->title); ?></strong>
          </div>

          <?php if($exam->description): ?>
            <div class="mb-3">
              <span class="text-muted d-block small mb-1">تفاصيل ونطاق الامتحان</span>
              <p class="m-0 small" style="color: var(--text-main);"><?php echo e($exam->description); ?></p>
            </div>
          <?php endif; ?>

          <div class="mb-3">
            <span class="text-muted d-block small mb-1">المادة</span>
            <strong style="color: var(--text-main);"><?php echo e($exam->subject->name ?? 'غير محدد'); ?></strong>
          </div>
          ...
          <div class="row mb-3">
            <div class="col-6">
              ...
              <strong style="color: var(--text-main);"
                dir="ltr"><?php echo e(\Carbon\Carbon::parse($exam->exam_date)->format('Y-m-d H:i')); ?></strong>
            </div>
            <div class="col-6">
              <span class="text-muted d-block small mb-1">المدة الزمنية</span>
              <strong style="color: var(--text-main);"><?php echo e($exam->duration_minutes); ?> دقيقة</strong>
            </div>
          </div>
          ...
          <div class="mb-4">
            ...
            <?php if($exam->is_published): ?>
              <span class="badge px-3 py-2 rounded-pill fw-normal"
                ...
                <i class="fa-solid fa-earth-americas me-1"></i> منشور للطلاب
              </span>
            <?php else: ?>
              <span class="badge px-3 py-2 rounded-pill fw-normal"
                style="background-color: rgba(245, 158, 11, 0.1); color: #f59e0b; border: 1px solid #f59e0b;">
                <i class="fa-solid fa-lock me-1"></i> مسودة غير منشور
              </span>
            <?php endif; ?>
          </div>
          ...
          <div class="d-grid gap-2">
            <?php if(!$exam->is_published): ?>
              <form action="<?php echo e(route('dashboard.exams.publish', $exam->id)); ?>" method="POST">
                <?php echo csrf_field(); ?>
            ...
            <a href="<?php echo e(route('dashboard.exams.edit', $exam->id)); ?>"
              class="btn rounded-pill py-2 fw-bold shadow-sm-hover mt-1"
        ...
        <div class="custom-card shadow-sm-hover border" style="border-color: var(--border-color) !important;">
          <h5 class="fw-bold mb-3" style="color: var(--text-main);">إجراءات الأسئلة والتسليمات</h5>
          <hr style="border-color: var(--border-color);">
        ...
        <div class="custom-card h-100 border" style="border-color: var(--border-color) !important;">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <h4 class="fw-bold m-0" style="color: var(--text-main);">الأسئلة المرتبطة بالامتحان
              (<?php echo e($exam->questions->count()); ?>)</h4>
          </div>
          <hr style="border-color: var(--border-color);">

          <div class="d-flex flex-column gap-3">
            <?php $__empty_1 = true; $__currentLoopData = $exam->questions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $question): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
              <div class="p-4 rounded-4 shadow-sm-hover"
                ...
                <div class="d-flex justify-content-between align-items-start mb-3">
                  <div>
                    <span class="badge rounded-pill px-3 py-2 me-2"
                      ...
                      <?php if($question->type == 'mcq'): ?> <i class="fa-solid fa-list-ul me-1"></i> خيارات متعددة
                      <?php elseif($question->type == 'TF'): ?> <i class="fa-solid fa-check-double me-1"></i> صح / خطأ
                      <?php else: ?> <i class="fa-solid fa-pen-nib me-1"></i> سؤال مقالي <?php endif; ?>
                    </span>
                    <span class="badge rounded-pill px-3 py-2"
                      style="background-color: rgba(163, 230, 53, 0.15); color: var(--accent-color);">
                      الدرجة: <strong><?php echo e($question->points); ?></strong>
                    </span>
                  </div>

                  
                  <form action="<?php echo e(route('dashboard.exams.questions.detach', [$exam->id, $question->id])); ?>" method="POST"
                ...
                <?php else: ?>
                  <div class="p-3 rounded-3"
                    ...
                    <span class="text-muted small d-block mb-2"><i class="fa-solid fa-pen text-info me-1"></i> نموذج الإجابة
                      الصحيحة:</span>
            ...
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
              <div class="text-center py-5 text-muted">
                ...
                لا توجد أسئلة مرتبطة بهذا الامتحان حالياً. يمكنك إنشاء سؤال جديد للبدء.
              </div>
            <?php endif; ?>
          </div>
        </div>
  ...
  <div class="modal fade" id="createQuestionModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content glass-modal text-end" dir="rtl"
        ...
        <form action="<?php echo e(route('dashboard.exams.questions.store', $exam->id)); ?>" method="POST">
          <?php echo csrf_field(); ?>
            ...
            <div class="row mb-4 g-3">
              ...
              <div class="col-md-6 text-start" dir="rtl">
                ...
                <input type="number" name="points" class="form-control bg-transparent rounded-3"
                  style="color: var(--text-main); border: 1px solid var(--border-color);" step="0.5" min="0" required
                  value="1">
              </div>
            </div>
              ...
              <div class="d-flex flex-column gap-3" id="choicesContainer">
                ...
                <div class="input-group choice-row">
                  ...
                  <input type="text" name="choices[0][text]" class="form-control bg-transparent"
                    style="color: var(--text-main); border-color: var(--border-color);" placeholder="نص الخيار رقم 1"
                    id="choice_input_0">
                </div>
                ...
                <div class="input-group choice-row">
                  ...
                  <input type="text" name="choices[1][text]" class="form-control bg-transparent"
                    style="color: var(--text-main); border-color: var(--border-color);" placeholder="نص الخيار رقم 2"
                    id="choice_input_1">
                </div>
              </div>

              <small id="choicesLimitText" class="text-danger mt-3 d-none fw-bold"><i
                  class="fa-solid fa-triangle-exclamation me-1"></i> وصلت للحد الأقصى المسموح (10 خيارات).</small>
            </div>
  ...
  <script>
    let choiceCount = 2; // نبدأ دائماً بخيارين كحد أدنى
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\USER\Desktop\school-for-blind-Backend\school_for_blind\resources\views/pages/exams/show.blade.php ENDPATH**/ ?>