...
class KickParticipantRequest extends BaseApiRequest
{
    public $roomModel;
    ...
    public function authorize()
    {
        ...
        if ($userType !== 'App\Models\Admin' && ($room->creator_id !== $user->id || $room->creator_type !== $userType)) {
            $this->authErrorMessage = 'غير مصرح لك! فقط مشرف النظام أو الأستاذ منشئ الدرس يمكنه الطرد';
            return false;
        }

        if ($userType === 'App\Models\Teacher' && $this->target_type === 'App\Models\Admin') {
            $this->authErrorMessage = 'غير مصرح لك بطرد مشرفي النظام (الإدارة)';
            return false;
        }
    ...
    public function messages()
    {
        return [
            ...
            'target_id.required' => 'يجب تحديد معرف الشخص المراد طرده',
            'target_type.required' => 'يجب تحديد نوع حساب الشخص المراد طرده',
