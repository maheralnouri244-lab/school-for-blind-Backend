...
<div class="container-fluid p-0">
  ...
  <ul class="nav nav-pills mb-4 gap-2" id="schedulesTabs" role="tablist">
    <li class="nav-item" role="presentation">
      <button class="nav-link active fw-bold px-4 py-2" id="classes-tab" data-bs-toggle="tab" data-bs-target="#classes-panel" type="button" role="tab">
        <i class="fa-solid fa-door-open ms-2"></i> أحدث جداول الشعب
      </button>
    </li>
    <li class="nav-item" role="presentation">
      <button class="nav-link fw-bold px-4 py-2" id="teachers-tab" data-bs-toggle="tab" data-bs-target="#teachers-panel" type="button" role="tab">
              ...
              @forelse($classSchedules as $schedule)
                ...
                <tr style="border-bottom: 1px solid var(--border-color);">
                  ...
                  <td class="py-3 text-start">
                    <button type="button" class="btn btn-sm btn-outline-primary ms-1" onclick="viewClassSchedule('{{ json_encode($contentData) }}', '{{ $schedule->title }}')">
                      <i class="fa-solid fa-eye me-1"></i> عرض
                    </button>

                    <form action="{{ route('dashboard.schedules.destroy', $schedule->id) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الجدول؟')">
                      @csrf
                      @method('DELETE')
            ...
            <tbody>
              @forelse($teachers as $teacher)
                <tr style="border-bottom: 1px solid var(--border-color);">
                  ...
                  <td class="py-3 text-start">
                    <button type="button" class="btn btn-sm btn-outline-success" onclick="viewTeacherSchedule('{{ $teacher->full_name }}')">
