  ...
  <div class="container-fluid p-0">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 p-3 rounded custom-card">
          ...
          <div class="d-flex gap-2">
              ...
              <button type="button" class="btn px-4 fw-bold" style="background-color: var(--accent-color); color: #000;" onclick="saveAllSchedules()">
      ...
      <div class="d-flex position-relative gap-3 overflow-hidden" style="min-height: 80vh;">
          <div class="flex-grow-1 position-relative overflow-hidden p-2 rounded custom-card" id="workspaceContainer" style="background-color: var(--bg-main); cursor: grab;">
              <div id="workspaceCanvas" style="width: 3500px; height: 2500px; position: absolute; top: 0; left: 0;">
                  @foreach($classes as $index => $class)
                    @php
                      ...
                      $top = 50 + ($row * 540);
                    @endphp
                    <div class="custom-card p-3 schedule-card position-absolute shadow-sm" 
                         id="class-card-{{ $class->id }}"
                         style="width: 580px; left: {{ $left }}px; top: {{ $top }}px; z-index: 10;" 
                         data-class-id="{{ $class->id }}" 
                         data-class-name="{{ $class->name }} - شعبة {{ $class->number }}">

                        <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2 card-header-drag cursor-move" style="cursor: move;">
                            <h5 class="fw-bold mb-0 text-primary">
                                <i class="fa-solid fa-up-down-left-right me-2 text-muted"></i> {{ $class->name }} (شعبة {{ $class->number }})
                            </h5>
                            <span class="badge bg-soft-info text-info">تفاعلي</span>
                        </div>

                        <div class="table-responsive">
                            <table class="table table-bordered text-center align-middle mb-0" style="font-size: 0.82rem; color: var(--text-main);">
                                <thead>
                                    <tr class="table-dark">
                                          ...
                                          @for($p = 1; $p <= 8; $p++)
                                            <td class="cell-slot cursor-pointer position-relative p-1" 
                                                ...
                                                <div class="slot-content d-flex flex-column justify-content-center h-100">
                                                    ...
                                                    <span class="teacher-text fw-bold text-success" style="font-size: 0.68rem;"></span>
                                                </div>
                                            </td>
                                          @endfor
                                      </tr>
          ...
          <div class="custom-card p-3 shadow-sm border-start" id="teacherDrawer" style="width: 320px; min-width: 320px; display: none; height: 80vh; overflow-y: auto; z-index: 100;">
              ...
              <div class="d-flex flex-column gap-2" id="teachersList">
                  @foreach($teachers as $teacher)
                    <div class="p-2 rounded border teacher-item" id="teacher-card-{{ $teacher->id }}" style="background-color: var(--bg-main);">
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="fw-bold" style="font-size: 0.85rem; color: var(--text-main);">{{ $teacher->full_name }}</span>
      ...
      <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content glass-modal">
              ...
              <div class="modal-footer d-flex justify-content-between">
                  <button type="button" class="btn btn-outline-danger btn-sm" onclick="clearCurrentSlot()">تفريغ الخلية</button>
                  <div>
                      <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">إلغاء</button>
                      <button type="button" class="btn btn-primary btn-sm px-3" onclick="applySlotAssignment()">تطبيق</button>
                  </div>
              </div>
          </div>
      </div>
...
@push('scripts')
  ...
  <script>
      ...
      function openAssignModal(classId, day, period, dayName) {
          ...
          if (current) {
              subjectSelect.value = current.subject;
              onSubjectChanged(current.teacherId);
          } else {
              subjectSelect.value = "";
              document.getElementById('teacherSelect').innerHTML = '<option value="">-- اختر مادة أولاً --</option>';
      ...
      function onSubjectChanged(selectedTeacherId = null) {
          ...
          let availableTeachers = allTeachers.filter(t => {
              let teachesSubject = t.subjects && t.subjects.some(s => s.name === subjectName);
              ...
              return teachesSubject && assignedToClass && notBusy;
          });
      ...
      function applySlotAssignment() {
          ...
          updateCellUI(classId, day, period, subject, teacherName);
      ...
      function saveAllSchedules() {
          ...
          document.querySelectorAll('.schedule-card').forEach(card => {
              ...
              let columns = ["الحصة", "الأحد", "الإثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
              let dayKeys = ["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"];

              let rows = [];

              for (let p = 1; p <= 8; p++) {
                  let rowObj = { period: `الحصة ${p}` };

                  dayKeys.forEach(dKey => {
                      let slot = getSlotData(dKey, p, classId);
                      rowObj[dKey] = slot ? `${slot.subject} (${slot.teacherName})` : '-';
                  });

                  rows.push(rowObj);
              }

              schedulesPayload[classId] = {
                  columns: columns,
                  rows: rows
              };
          });
