   ...
   <form action="{{ route('reports.index') }}" method="GET" class="row g-3 align-items-center">
    <div class="col-md-4">
     ...
     <select name="status" class="form-select bg-transparent text-main border-color" onchange="this.form.submit()">
      <option value="pending" {{ request('status') == 'pending' || !request('status') ? 'selected' : '' }}>معلق (Pending)
      </option>
      <option value="reviewed" {{ request('status') == 'reviewed' ? 'selected' : '' }}>تمت المراجعة (Reviewed)</option>
      <option value="dismissed" {{ request('status') == 'dismissed' ? 'selected' : '' }}>مرفوض/متجاهل (Dismissed)</option>
     </select>
    </div>
    <div class="col-md-4">
     <label class="form-label text-muted">نوع المُبلّغ عليه</label>
     <select name="user_type" class="form-select bg-transparent text-main border-color" onchange="this.form.submit()">
     ...
     <tbody>
      @forelse($reports as $report)
       <tr style="border-bottom: 1px solid var(--border-color);">
        <td class="py-3">
         <span class="fw-bold">{{ $report->reporter->fullname ?? $report->reporter->full_name ?? 'مستخدم محذوف' }}</span>
         <small
          class="text-muted d-block">{{ class_basename($report->reporter_type) == 'Student' ? 'طالب' : 'أستاذ' }}</small>
        </td>
        <td class="py-3">
         <span
          class="fw-bold text-danger">{{ $report->reported->fullname ?? $report->reported->full_name ?? 'مستخدم محذوف' }}</span>
         <small
          class="text-muted d-block">{{ class_basename($report->reported_type) == 'Student' ? 'طالب' : 'أستاذ' }}</small>
        </td>
        ...
        <div class="modal-dialog modal-md">
         <div class="modal-content text-end" dir="rtl">
          <div class="modal-header d-flex justify-content-between">
           <h5 class="modal-title fw-bold" style="color: var(--text-main);">معالجة البلاغ رقم #{{ $report->id }}</h5>
          ...
          <div class="modal-body">
           <div class="mb-4 p-3 rounded"
            ...
            <p class="text-muted mb-1"><strong>المُبلِغ:</strong>
             {{ $report->reporter->fullname ?? $report->reporter->full_name ?? '-' }}</p>
            <p class="text-muted mb-1"><strong>المُبلَغ عليه:</strong> <span
              class="text-danger">{{ $report->reported->fullname ?? $report->reported->full_name ?? '-' }}</span></p>
             ...
             <div class="mb-3">
              ...
              <select name="punishment_id" class="form-select bg-transparent text-main border-color" required>
               ...
               @foreach($punishments as $punishment)
                <option value="{{ $punishment->id }}">{{ $punishment->name }} (مستوى {{ $punishment->level }})</option>
               @endforeach
              </select>
             </div>
